...
#   Minimal grouped GEMM placeholder.

#    routed_token_buffer: [N_tokens, hidden_dim]
#    expert_weights:      [num_experts, hidden_dim, expert_dim]

#    A single ttry here which could be replaced to some real grouped GEMM kernel later
    assert routed_token_buffer.dim() == 2
    assert expert_weights.dim() == 3

    hidden_dim = routed_token_buffer.shape[-1]
    _, h_w, expert_dim = expert_weights.shape
    assert h_w == hidden_dim, "Hidden dim mismatch between tokens and expert weights"

    # [hidden_dim, expert_dim]
    weight_eff = expert_weights.mean(dim=0)

    # [N_tokens, expert_dim]
    return routed_token_buffer @ weight_eff
